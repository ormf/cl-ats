...
(defun tracker (file snd &key 	
  ...
  (let* (;;; input file
	 ...
	 (nd (if duration (+ st (floor (* duration file-sampling-rate)))(sound-frames fil)))
	 ;;; number of samples to read
	 (total-samps (- nd st))
         ;;; file duration
	 (file-duration (double-float (/ total-samps file-sampling-rate)))
	 ;;; number of samples in a cycle
	 (cycle-samps (floor (* (/ 1 lowest-frequency) window-cycles file-sampling-rate)))
